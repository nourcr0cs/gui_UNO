...
public abstract class Card {
	...
	/*
	 * public boolean isCompatible(Card comparator) { return
	...
	public boolean nextNextPlayer(Card card) {
		return card.getType().equalsIgnoreCase("skip") || card.getType().equalsIgnoreCase("draw")
				|| card.getType().equalsIgnoreCase("wild") || card.getValue().equals("+2");
	}

	// to check if we should to reverse the game

	public boolean isReverseCard() {
		return this.getType().equalsIgnoreCase("special") && this.getValue().equalsIgnoreCase("Reverse");
	}
	...
	/*
	 * public static class WildCard extends Card { public WildCard(String value) {
	 ...
	 * 
	 * // use the methode of class game game.wildEffect(player, deck);
	 * 
	 * } }
	 */
