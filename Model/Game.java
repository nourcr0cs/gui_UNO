...
public class Game {
	...
	// Modified initPlayers method to automatically set player 1 as "You" (human
	// player)
			...
			if (playableCards == 0) {
				...
				if (drawnCard.isPlayable(topDiscardPile, colorToPlay)) {
					...
					if (currentPlayer instanceof Human) {
						...
						System.out.println("Enter the number of the card you want to play (1-"
								+ currentPlayer.getHandSize() + ") or 'skip' to skip your turn:");
	...
	private String chooseColor() {
		...
		while (true) {
			...
			if (color.equalsIgnoreCase("Yellow") || color.equalsIgnoreCase("Red") || color.equalsIgnoreCase("Blue")
	 ...
	 * pileOfGame.push(card); setTopCard(card); setColorToPlay(card.getColor());
	 * player.removeFromHand(card); card.effectCard(player, deck, this);
	 * //checkWinner(player); //advanceToNextPlayer(false); }
	 */

	public void jouerCarteDepuisUI(Player player, Card card) {
		player.removeCard(card); // ✅ retirer la carte jouée
		this.topCard = card;
	...
	/*
	 * public void StartGame() { //initPlayers(players); shuffleDeck();
	 * distributeCards();
	 * 
	 * this.topCard = deck.drawingFromDeck(pileOfGame); // 💥 manquait ici ! if
	 * (this.topCard == null) { throw new
	 * IllegalStateException("Impossible de démarrer : aucune carte disponible !");
	 * } pileOfGame.push(topCard); }
	 */
	...
	public void StartGame() {
		...
		// Piocher et définir la carte du dessus
		this.topCard = deck.drawingFromDeck(pileOfGame);
		if (this.topCard == null) {
			throw new IllegalStateException("Erreur : aucune carte n'a pu être tirée.");
		}
		pileOfGame.push(topCard);
	}
