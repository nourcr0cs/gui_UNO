		...
		cardContainer = new JPanel() {
			...
			private void layoutHorizontalCards(int numCards) {
				...
				for (int i = 0; i < numCards; i++) {
					...
					card.setBounds(startX + (i * effectiveCardWidth) - margin, (getHeight() - CARD_HEIGHT) / 2 - margin,
							CARD_WIDTH + (margin * 2), CARD_HEIGHT + (margin * 2));
				}
			}
			...
			private void layoutVerticalCards(int numCards) {
				...
				for (int i = 0; i < numCards; i++) {
					...
					card.setBounds((getWidth() - verticalCardHeight) / 2 - marginX,
							startY + (i * effectiveCardHeight) - marginY, verticalCardHeight + (marginX * 2),
