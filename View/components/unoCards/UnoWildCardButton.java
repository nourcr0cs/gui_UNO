...
public class UnoWildCardButton extends UnoCardButton {
	...
	public UnoWildCardButton(WildType wildType) {
		/*
		 * //super(UnoColor.BLACK); super(UnoColor.BLACK, wildType.toString(), "wild");
		 * // ✅ C'est bien wildType this.wildType = wildType; this.selectedColor = null;
		 */
	...
	private void drawWildCard(Graphics2D cmp, int width, int height) {
		...
		cmp.fillArc(centerX - circleRadius, centerY - circleRadius, colorCircleSize, colorCircleSize, 90, 90);

		cmp.setColor(UnoColor.BLUE.getColor());
		cmp.fillArc(centerX - circleRadius, centerY - circleRadius, colorCircleSize, colorCircleSize, 0, 90);

		cmp.setColor(UnoColor.YELLOW.getColor());
		cmp.fillArc(centerX - circleRadius, centerY - circleRadius, colorCircleSize, colorCircleSize, 180, 90);

		cmp.setColor(UnoColor.GREEN.getColor());
		cmp.fillArc(centerX - circleRadius, centerY - circleRadius, colorCircleSize, colorCircleSize, 270, 90);

		cmp.setColor(Color.BLACK);
		cmp.setStroke(new BasicStroke(1f));
		cmp.drawOval(centerX - circleRadius, centerY - circleRadius, colorCircleSize, colorCircleSize);
	...
	private void drawWildDrawFour(Graphics2D cmp, int width, int height) {
		...
		if (selectedColor == null) {
			// Yellow card (top)
			cmp.setColor(UnoColor.YELLOW.getColor());
			drawCardWithShadow(cmp, centerX - cardWidth / 2, centerY - cardHeight - spacing / 2, cardWidth, cardHeight);

			// Blue card (left)
			cmp.setColor(UnoColor.BLUE.getColor());
			drawCardWithShadow(cmp, centerX - cardWidth - spacing, centerY - cardHeight / 2, cardWidth, cardHeight);

			// Red card (right)
			cmp.setColor(UnoColor.RED.getColor());
			drawCardWithShadow(cmp, centerX + spacing / 2, centerY - cardHeight / 2, cardWidth, cardHeight);

			// Green card (bottom)
			cmp.setColor(UnoColor.GREEN.getColor());
			drawCardWithShadow(cmp, centerX - cardWidth / 2, centerY + spacing / 2, cardWidth, cardHeight);
		} else {
			// draw single colored card with +4 text
			cmp.setColor(selectedColor.getColor());
			drawCardWithShadow(cmp, centerX - cardWidth / 2, centerY - cardHeight / 2, cardWidth, cardHeight);
	...
	private void drawCardWithShadow(Graphics2D cmp, int x, int y, int width, int height) {
		...
		/*
		 * cmp.setColor(Color.BLACK); cmp.fillRoundRect(x + 2, y + 2, width, height, 6,
		 * 6);
		 */
