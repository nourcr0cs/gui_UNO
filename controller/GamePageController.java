...
import Modell.Bot;
import Modell.SimpleCard;
import Modell.SpecialCard;
import Modell.WildCard;
...
public class GamePageController {
      ...
      private Game game;  
      private GameWithCards view; // ou GameView selon le nom de ta classe vue
      ...
      public GamePageController(Game game, GameWithCards view) {
            this.game = game;
            this.view = view;
            game.StartGame();
        }
     ...
     /*  public void onCardClicked(UnoCardButton button) {
      Card modelCard = button.getAssociatedCard(); // 👈 transforme en carte modèle
    ...
    if (modelCard.isPlayable(game.getTopCard(), game.getColorToPlay())) {
        // logique métier
        current.removeCard(modelCard);
        game.getPileOfGame().push(modelCard);
        game.setTopCard(modelCard);
        game.setColorToPlay(modelCard.getColor());

        modelCard.effectCard(current, game.getDeck(), game);

        // met à jour la vue
        view.setDiscardPileCard(button); // on peut réutiliser le bouton cliqué
        view.setPlayerCards(0, generateCardButtons(current.getHand())); // recréer les boutons
...
public UnoCardButton convertCardToButton(Card card) {
    ...
    if (card instanceof SimpleCard) {
        ...
        btn = new UnoNumberCardButton(simple.getColor(), simple.getNumber());
    } else if (card instanceof SpecialCard) {
        SpecialCard special = (SpecialCard) card;
        btn = new UnoActionCardButton(special.getColor(), special.getActionType());
    } else if (card instanceof WildCard) {
        WildCard wild = (WildCard) card;
        btn = new UnoWildCardButton(wild.getWildType());
    } else {
        throw new IllegalArgumentException("Type de carte inconnu");
    }

    btn.setAssociatedCard(card);
    return btn;
}*/
 ...
 public  Card convertCardButton(UnoCardButton button) {
      if (button instanceof UnoNumberCardButton) {
          ...
          return new SimpleCard(
              Integer.toString(numberButton.getNumber()),  // La valeur de la carte
          ...
          return new SpecialCard(
              actionButton.getActionType().toString(),   // Le type d'action comme chaîne de caractères
          ...
          return new WildCard(
              wildButton.getWildType().toString()  // Le type de carte Wild comme chaîne de caractères
          );
      }
...
/* 
  // Convert UnoColor to Modell.Color
  private Color toModelColor(UnoColor unoColor) {
      switch (unoColor) {
  ...
  */
  ...
  private UnoColor toUnoColor(String color) {
      switch (color.toUpperCase()) {
          ...
          case "YELLOW": return UnoColor.YELLOW;
          case "BLACK": return UnoColor.BLACK;
 ...
 /* 
  public UnoCardButton convertCardToButton(Card card) {
      ...
      if (card instanceof SimpleCard) {
          ...
          btn = new UnoNumberCardButton(color, Integer.parseInt(simple.getValue()));
  
      } else if (card instanceof SpecialCard) {
          SpecialCard special = (SpecialCard) card;
          UnoColor color = toUnoColor(special.getColor());
          UnoActionCardButton.ActionType type = UnoActionCardButton.ActionType.valueOf(special.getValue().toUpperCase());
          btn = new UnoActionCardButton(color, type);
  
      } else if (card instanceof WildCard) {
          WildCard wild = (WildCard) card;
          UnoWildCardButton.WildType type = UnoWildCardButton.WildType.valueOf(wild.getValue().toUpperCase());
          btn = new UnoWildCardButton(type);
...
/*public void onCardClicked(Card card) {
      ...
      if (!current.isMoveValid(card, topCard, game.getColorToPlay())) {
          return; // Clique invalide
      }
  
      // Signaler la carte jouée à la logique du jeu
      game.jouerCarteDepuisUI(current, card); // ← méthode à créer dans Game (si elle n'existe pas déjà)
      ...
      // Laisser le jeu gérer le tour suivant
  ...
  private void jouerTourBot() {
      ...
      new Timer().schedule(new TimerTask() {
          ...
          public void run() {
              ...
              // 1. Le bot cherche une carte jouable
              Card move = bot.selectCardToPlay(topCard, colorToPlay);
  
              if (move != null) {
                  // ✅ Il peut jouer → joue directement
                  onCardClicked(move);
              } else {
                  // ❌ Il ne peut pas jouer → pioche
                  Card drawn = game.getDeck().drawingFromDeck(game.getPileOfGame());
                  bot.addCard(drawn);
                  view.updateHands(game.getPlayers());
  
                  // 2. Vérifie si la carte piochée est jouable
                  if (drawn.isPlayable(topCard, colorToPlay)) {
                      // ✅ Elle est jouable → joue immédiatement
                      onCardClicked(drawn);
                  } else {
                      // ❌ Elle n’est pas jouable → passer au joueur suivant
                      game.advanceToNextPlayer(false);
                      view.updateActivePlayer(game.getCurrentPlayerIndex());
  
                      // 🔁 Recommence si le suivant est aussi un bot
                      if (game.getCurrentPlayer() instanceof Bot) {
                          jouerTourBot(); // récursif mais avec Timer donc pas bloquant
                      }
                  }
              }
          }
      }, 1500); // délai pour effet visuel
  }
  ...
  public void onDrawClicked() {
      ...
      // Tirer une carte
      Card drawnCard = game.getDeck().drawingFromDeck(game.getPileOfGame());
      current.addCard(drawnCard);
  
      // Mettre à jour la main dans la vue
      view.updateHands(game.getPlayers());
  
      // Vérifier si la carte piochée est jouable
      if (drawnCard.isPlayable(game.getTopCard(), game.getColorToPlay())) {
          // Si c’est un bot → jouer directement
          if (current instanceof Bot) {
              onCardClicked(drawnCard);
          } else {
              // Si humain, laisser le joueur jouer manuellement
              System.out.println("Carte piochée jouable. Tu peux la jouer.");
          }
      } else {
          // Si la carte n’est pas jouable, passer au joueur suivant
          game.advanceToNextPlayer(false);
          view.updateActivePlayer(game.getCurrentPlayerIndex());
  
          // Si prochain joueur est un bot, jouer automatiquement
          if (game.getCurrentPlayer() instanceof Bot) {
              jouerTourBot();
          }
      }
  ...
  public void gererTourJoueur() {
      ...
      if (current instanceof Bot) {
          jouerTourBot(); // bot gère tout seul
      } else {
            if (current.hasPlayableCard(topCard, colorToPlay)) {
                  // Joue automatiquement la première carte jouable
                  Card cardToPlay = current.selectCardToPlay(topCard, colorToPlay);
                  onCardClicked(cardToPlay);
              } else {
...
/* 
  public void appliquerTour(Card card) {
    ...
    if (card instanceof SpecialCard) {
        ...
        if (value.equals("+4")) {
            Player next = game.getNextPlayer();
            for (int i = 0; i < 4; i++) next.drawCard(game.getDeck(), game.getPileOfGame());
            view.updateHands(game.getPlayers());
        ...
        if (current instanceof Human) {
            view.showColorChooser(chosenColor -> {
                ...
                view.updateActivePlayer(game.getCurrentPlayerIndex());
                gererTourJoueur(); // continue le jeu
            });
            return; // Attente du clic
        } else {
            String autoColor = current.chooseColor(); // implémente ça si besoin
...
public void onCardClicked(Card card) {
      Player current = game.getCurrentPlayer();
      if (!current.isMoveValid(card, game.getTopCard(), game.getColorToPlay())) return;
...
public void onCardClicked(Card card) {
      ...
      if (!valide) {
          System.out.println("❌ Mouvement invalide : " + card.getValue() + " sur " + game.getTopCard().getValue() + " avec couleur " + game.getColorToPlay());
...
/* 
  ...
  public void restartGame() {
      ...
      for (Player p : game.getPlayers()) {
          Player nouveau = (p instanceof Bot)
              ? new Bot(null, null, p.getName())
              : new Human(null, null, p.getName());
