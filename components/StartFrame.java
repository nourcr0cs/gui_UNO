...
public class StartFrame extends JFrame {
    ...
    /*private JLabel createUnoLogoLabel() {
        JLabel label = new JLabel("UNO", SwingConstants.CENTER) {
            @Override
            protected void paintComponent(Graphics g) {
                super.paintComponent(g);
                Graphics2D g2d = (Graphics2D) g.create();
                ...
                // Get the text bounds for positioning
                ...
                // Draw black shadow first (offset slightly down-right)
                g2d.setFont(getFont());
                g2d.setColor(Color.BLACK);
                int shadowOffset = 5;
                g2d.drawString(getText(), x + shadowOffset, y + shadowOffset);
                
                // Draw thick black outline
                g2d.setColor(Color.BLACK);
                for (int i = 1; i <= 3; i++) {
                    g2d.drawString(getText(), x - i, y);     // left
                    g2d.drawString(getText(), x + i, y);     // right
                    g2d.drawString(getText(), x, y - i);     // top
                    g2d.drawString(getText(), x, y + i);     // bottom
                    g2d.drawString(getText(), x - i, y - i); // top-left
                    g2d.drawString(getText(), x + i, y - i); // top-right
                    g2d.drawString(getText(), x - i, y + i); // bottom-left
                    g2d.drawString(getText(), x + i, y + i); // bottom-right
                }
                
                // Draw white border between black outline and yellow fill
                g2d.setColor(Color.WHITE);
                for (int i = 1; i <= 2; i++) {
                    // Inner white border (smaller than the black outline)
                    g2d.drawString(getText(), x - i + 1, y);
                    g2d.drawString(getText(), x + i - 1, y);
                    g2d.drawString(getText(), x, y - i + 1);
                    g2d.drawString(getText(), x, y + i - 1);
                    g2d.drawString(getText(), x - i + 1, y - i + 1);
                    g2d.drawString(getText(), x + i - 1, y - i + 1);
                    g2d.drawString(getText(), x - i + 1, y + i - 1);
                    g2d.drawString(getText(), x + i - 1, y + i - 1);
                }
                
                // Draw the yellow fill on top
                g2d.setColor(new Color(255, 204, 20)); 
        ...
        label.setFont(new Font("Arial", Font.BOLD | Font.ITALIC, 90));
