...
import Modell.Player; // si ce n’est pas déjà importé
    ...
    for (UnoCardButton cardButton : cards) {
        cardButton.addActionListener(e -> {
            if (controller != null) {
                ...
                controller.onCardClicked(cardButton.getAssociatedCard()); // 👉 envoyer le bouton, pas décider
...
/* 
  // bch ybdel top card
   ...
   /* 
    public void updateHands(java.util.List<Modell.Player> players) {
        for (int i = 0; i < players.size(); i++) {
            Modell.Player player = players.get(i);
    ...
    public void showGameOver(String winnerName) {
        JOptionPane.showMessageDialog(
            this,
            "🏆 Le joueur gagnant est : " + winnerName + " 🎉",
            "Fin de la partie",
     ...
     /*public void showColorChooser(Consumer<String> callback) {
        ColorChooserOverlay overlay = new ColorChooserOverlay(callback);
        this.add(overlay); // `this` = JFrame ou JPanel principal
        this.repaint();
        this.revalidate();
    }
    */
...
public void updateHands(List<Player> players) {
    for (int i = 0; i < players.size(); i++) {
        ...
        if (player instanceof Human) {
            UnoCardButton[] buttons = controller.generateCardButtons(player.getHand());
            setPlayerCards(i, buttons); // méthode qui ajoute les boutons dans un JPanel
        } else {
            setPlayerCards(i, new UnoCardButton[0]); // rien pour les bots
        }
    }
}*/
...
public void updateHands(List<Player> players) {
    for (int i = 0; i < players.size(); i++) {
        ...
        if (player == game.getCurrentPlayer()) {
            // Joueur courant → afficher ses vraies cartes
            UnoCardButton[] buttons = controller.generateCardButtons(player.getHand());
            setPlayerCards(i, buttons);
        } else {
            // Autres joueurs → afficher dos de carte (ex: 7 cartes)
            int count = player.getHand().size();
            UnoCardButton[] hidden = new UnoCardButton[count];
            for (int j = 0; j < count; j++) {
                hidden[j] = new UnoCardBackButton(); // bouton avec image du dos de carte
            }
            setPlayerCards(i, hidden);
        }
    }
}*/
...
private Modell.Game game;

public void setGame(Modell.Game game) {
    this.game = game;
}
